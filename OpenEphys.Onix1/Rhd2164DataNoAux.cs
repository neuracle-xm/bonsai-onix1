                            ...
                            frame =>
                            {
                                ...
                                else if (GlobalState.HubStates[GlobalState.DeviceNameToHubName[DeviceName]] == HubState.Impedance)
                                {
                                    if (++sampleIndex >= bufferSize)
                                    {
                                        ...
                                        if (l1_mean != 0)
                                        {
                                            r1 = (v1_mean - v2_mean) / l1_mean;
                                            r2 = v2_mean / l1_mean;
                                        }
                                        observer.OnNext(new Rhd2164DataFrameNoAux(clockBuffer, hubClockBuffer, Mat.Zeros(Rhd2164NoAux.ImpedanceChannelCount, bufferSize, Depth.S32, 1), r1, r2));
                                        sampleIndex = 0;
                                    }
                                }
                                //刺激模式不用做什么
